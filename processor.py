...
def lonlat_bbox_from_center(lon: float, lat: float, scale_m: float) -> List[float]:
    ...
    return [round(lon - dlon / 2, 6), round(lat - dlat / 2, 6), round(lon + dlon / 2, 6), round(lat + dlat / 2, 6)]
...
def build_masked_composite(period: Period, region: ee.Geometry) -> ee.Image:
    collection = ee.ImageCollection(DATASET).filterDate(period.start, period.end).filterBounds(region)
...
def compute_direction_summary(pixels: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for name, target in targets.items():
        ...
        if selected:
            ...
            out.append({"name": name, "bearing": round(tb, 1), "pixels": len(vals), "mean": round(sum(vals) / len(vals), 4), "max": round(max(vals), 4)})
    return out
...
def build_result(period: Period, region: ee.Geometry) -> Dict[str, Any]:
    ...
    return {
        ...
        "meta": {
            ...
            "placeholder": False,
            "interpretation": "Vrednosti su noćna radijansa u nW/cm²/sr, a ne SQM mag/arcsec². Javni heatmap prikaz je izglađena vizuelizacija.",
        },
...
def main() -> int:
    ...
    for period in periods:
        ...
        index_periods.append({"id": period.id, "label": period.label, "source": "NASA Black Marble VNP46A2"})
    ...
    index = {
        ...
        "coverage": {"count": len(index_periods), "first": periods[0].id, "last": periods[-1].id},
